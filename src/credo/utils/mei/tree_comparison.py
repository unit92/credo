...
class TreeComparison(ComparisonStrategy):
    ...
    def compare_trees(self, a: et.ElementTree, b: et.ElementTree) \
        ...
        diff = self.__get_diff(a_transformer.tree, b_transformer.tree)
    ...
    def __get_diff(self, a: et.ElementTree, b: et.ElementTree, ):

        # Define functions for handline different types of xmldiff actions
        action_map = {
            actions.UpdateAttrib: self.__process_update,
            actions.DeleteAttrib: self.__process_update,
            actions.InsertAttrib: self.__process_update,
            actions.DeleteNode: self.__process_deletion
        }
        ...
        for diff_action in diff_actions:
            # Ignore all actions that are not supported by the
            # comparison algorithm.
            print(diff_action)
            if type(diff_action) not in action_map:
                continue

            process_action = action_map.get(type(diff_action))
            process_action(diff, a, b, diff_action)
